...
export const songArtists = (song) => {
  ...
  const add = (artist, fallbackName) => {
    ...
    out.push({
      ...
      isVerified:
        typeof artist === "string" ? false : Boolean(artist?.isVerified),
    });
  };
...
/**
 * The credited artists who actually have a page to open — the verified ones.
 *
 * An unverified account is a credit rather than an artist here: a one-off
 * feature, or a name lifted out of a filename. The name still appears under
 * the title, because leaving a collaborator off their own track would be a
 * lie. What it does not do is behave like a link to somewhere.
 *
 * Use this anywhere a tap leads to /artist/:id. `songArtists` stays the
 * unfiltered list, for anything that is only displaying names.
 */
export const songOpenableArtists = (song) =>
  songArtists(song).filter((artist) => artist.isVerified);
