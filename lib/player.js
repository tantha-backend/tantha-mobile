    ...
    async (position, list) => {
      ...
      try {
        ...
        try {
          ...
          // Premium-only tracks and network blips fall back to whatever the
          // list already carried, which may still be a valid signed URL.
          url = song.audio320 || null;
        }
        ...
        // Lock screen metadata is a bonus, not part of playing the track, so
        // a rejection here must not surface as a failed play. Under Expo Go
        // it always fails — see the note in app/_layout.js.
        try {
          ...
          // Audio is already playing; the lock screen just stays plain.
        }
      } finally {
