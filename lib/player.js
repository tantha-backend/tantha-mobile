...
export const PlayerProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const keep = [current?.audio320, url].filter(Boolean);

    for (const held of preloaded.current) {
      if (!keep.includes(held)) clearPreloadedSource(held).catch(() => {});
    }

    preloaded.current = preloaded.current.filter((held) => keep.includes(held));
  }, [current, queue, followingIndex]);
