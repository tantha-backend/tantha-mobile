...
export const authService = {
   ...
   *
   * The server verifies Google's signature and decides whether this is a new
   * listener or an existing account — the app only carries the token across.
   */
  ...
  /**
   * The same exchange for Apple.
   *
   * `fullName` is sent alongside the token because Apple does not put it in
   * the token, and only offers it at all on the very first authorisation.
   * The server uses it to name a new account and ignores it otherwise.
   */
  apple: async (identityToken, fullName) => {
   ...
   *
   * Confirmed with the password, or — for accounts that signed up through
   * Google and therefore have none — by typing DELETE. The server decides
   * which it requires and rejects the request if the wrong one arrives, so
   * the screen cannot get this wrong on someone's behalf.
   */
